...
module.exports = {
  insertPolice(req, res) {
    ...
    userDao.selectPolice(
      [NAME_OF_INVESTIGATION_AGENCY, INVESTIGATIVE_AGENCY_LEVEL],
      (err, data) => {
        if (err) {
          res.status(500).send();
        } else {
          if (data.length !== 0) {
            console.log(
              '数据库内已存在该侦查机关，名称是：',
              data[0].NAME_OF_INVESTIGATION_AGENCY
            );
            //将数据库中侦查机关的编号与判决书的编号绑定
            userDao.bindPolicenum(
              [data[0].INVESTIGATION_AGENCY_NUMBER, JUDGMENT_NUMBER],
              (err, data) => {
                if (err) {
            ...
            let INVESTIGATION_AGENCY_NUMBER =
              'Gongan' + randomNum.randomNumber(); //生成侦查机关编号
            userDao.registerPolice(
              [
                INVESTIGATION_AGENCY_NUMBER,
                NAME_OF_INVESTIGATION_AGENCY,
                INVESTIGATIVE_AGENCY_LEVEL,
              ],
              (err) => {
                if (err) {
                  ...
                  userDao.bindPolicenum(
                    [INVESTIGATION_AGENCY_NUMBER, JUDGMENT_NUMBER],
                    (err, data) => {
                      if (err) {
                        console.log(
                          '判决书编号绑定公安机关编号出错，err是：',
                          err
                        );
                        res.status(500).send();
                      } else {
